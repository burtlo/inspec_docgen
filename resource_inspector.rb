...
# module. Eventually we would want to iterate over the collection of all
# the constants but for right now, let's look at a single one.
#
# puts resources

# You said you wanted to see the usage of the KernelParameter. So we don't
# need the Array of `resources` just yet.

# We could hard-code this class name ...
Inspec::Resources::KernelParameter

# But eventually we would need to use the values defined in the Array above.
...
# Eventually we would do something like ...
...
# With a class/module we can ask for all its public and protected instance
# methods ....
...
# We probably should remove `to_s`. Ruby treats Arrays like sets. So we
...
# of a classes ancestors ... but we'll leave that as a @todo
...
# We could output them. All arrays have a method called `join` which takes a
...
# Now that we have the list of matchers we can subtract them
...
Inspec::Resource.registry.each do |resource_name, resource_class|
  # However, this resource_class is actually a descendent of the original class
  # that you want. So we cannot ask for the instance_methods on it. We need
  # to traverse one level up to its superclass.
